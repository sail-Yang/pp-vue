...
<script>
import Chart from '@/views/weather/direction/DirectionChart'
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      getWeatherByPeriod(beginTime, endTime, this.form.fan, 'winddirection').then(
        response => {
          sessionStorage.setItem('directionXdata', JSON.stringify(response.data))
    ...
    handleDownload() {
      this.xdata = JSON.parse(sessionStorage.getItem('directionXdata'))
      this.downloadLoading = true
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['datatime', 'windDirection']
        const filterVal = ['datatime', 'windDirection']
        const list = this.xdata.fanDataList
        const data = this.formatJson(filterVal, list)
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: this.exportform.filename || '风向',
