...
<script>
import Chart from '@/views/weather/wind/WindChart'
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      getWeatherByPeriod(beginTime, endTime, this.form.fan, 'wind').then(
        response => {
          sessionStorage.setItem('windXdata', JSON.stringify(response.data))
    ...
    handleDownload() {
      this.xdata = JSON.parse(sessionStorage.getItem('windXdata'))
      this.downloadLoading = true
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['datatime', 'windSpeed', 'ws']
        const filterVal = ['datatime', 'windSpeed', 'ws']
        const list = this.xdata.fanDataList
        const data = this.formatJson(filterVal, list)
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: this.exportform.filename || '风速',
