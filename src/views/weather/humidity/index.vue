    ...
    <el-row>
      <el-form :model="form" label-width="80px" :inline="true">
        <el-row :gutter="3" type="flex" align="middle">
          ...
          <el-col :xs="{span: 5}" :sm="{span: 2}" :lg="{span: 2}" />
        </el-row>
        <el-row>
...
<script>
import Chart from '@/views/weather/humidity/HumidityChart'
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      getWeatherByPeriod(beginTime, endTime, this.form.fan, 'humidity').then(
        response => {
          sessionStorage.setItem('humidityXdata', JSON.stringify(response.data))
    ...
    handleDownload() {
      this.xdata = JSON.parse(sessionStorage.getItem('humidityXdata'))
      this.downloadLoading = true
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['datatime', 'humidity']
        const filterVal = ['datatime', 'humidity']
        const list = this.xdata.fanDataList
        const data = this.formatJson(filterVal, list)
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: this.exportform.filename || '湿度',
