...
<script>
import Chart from '@/views/weather/temperature/TemperatureChart'
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['datatime', 'temperature']
        const filterVal = ['datatime', 'temperature']
        const list = this.xdata.fanDataList
        const data = this.formatJson(filterVal, list)
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: this.exportform.filename || '温度',
