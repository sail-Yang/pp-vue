...
export default {
  ...
  mounted() {
    this.loading = true
    if (sessionStorage.getItem('temperatureXdata') !== null) {
      this.xdata = JSON.parse(sessionStorage.getItem('temperatureXdata'))
    } else {
      getWeatherByRealTime(1).then(
        response => {
          sessionStorage.setItem('temperatureXdata', JSON.stringify(response.data))
          this.xdata = JSON.parse(sessionStorage.getItem('temperatureXdata'))
        }
      ).catch(() => {
        this.$message({
    ...
    initChart() {
      ...
      this.chart.setOption({
        ...
        series: [{
          ...
          areaStyle: {
            normal: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                offset: 0,
                color: 'rgba(188, 139, 198, 0.3)'
              }, {
                offset: 0.8,
                color: 'rgba(188, 139, 198, 0)'
              }], false),
          ...
          itemStyle: {
            normal: {
              color: 'rgb(188, 139, 198)',
              borderColor: 'rgba(188, 139, 198,0.2)',
              borderWidth: 12
            }
          },
